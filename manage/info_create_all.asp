...
sub infocreateAll()
	...
	if not (rs.eof and rs.bof) then
		response.write"<b><font color=#ff0000>正在生成INFO的Html页,请等待......</font></b><span id=showImport></span><IE:Download ID=""oDownload"" STYLE=""behavior:url(#default#download)"" />"
		dim i,bn,en
		i=0
		do while not rs.eof
			iid=clng(rs("iid"))
	...
	if en>=clng(recordcount) then
		Response.Write "<meta http-equiv=refresh content='0;url=info_create_all.asp?act=ok&pageno="&en&"&time="&time&"'>"
	else
		Response.Write "<meta http-equiv=refresh content='0;url=info_create_all.asp?act="&act&"&pageno="&pageno+1&"&recordcount="&recordcount&"&time="&time&"&cid="&cid&"&s="&en
		select case act
		case "period"
...
sub createhtml()
	Response.Write "<script>function onDownloadDone(downDate){showImport.innerHTML=downDate}oDownload.startDownload('info_create.asp?iid="&iid&"',onDownloadDone)</script>"
end sub
%>