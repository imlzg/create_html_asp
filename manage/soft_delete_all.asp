...
sub softdeleteAll()
	...
	if not (rs.eof and rs.bof) then
		response.write"<b><font color=#ff0000>正在删除SOFT的Html页,请等待......</font></b><span id=showImport></span><IE:Download ID=""oDownload"" STYLE=""behavior:url(#default#download)"" />"
		dim i,bn,en
		i=0
		do while not rs.eof
			sid=clng(rs("sid"))
	...
	if en>=clng(recordcount) then
		...
		Response.Write "<meta http-equiv=refresh content='0;url=soft_delete_all.asp?act=ok&pageno="&en&"&time="&time&"&onlyhtml="&onlyhtml&"'>"
	else
		Response.Write "<meta http-equiv=refresh content='0;url=soft_delete_all.asp?act="&act&"&pageno="&pageno+1&"&recordcount="&recordcount&"&time="&time&"&cid="&cid&"&s="&en&"&onlyhtml="&onlyhtml
		select case act
		case "period"
...
sub success()
	Response.Write "<br>批量删除SOFT页<br><br>操作成功：共"
	if onlyhtml<>1 then
...
sub deletehtml()
	Response.Write "<script>function onDownloadDone(downDate){showImport.innerHTML=downDate}oDownload.startDownload('soft_delete.asp?sid="&sid&"',onDownloadDone)</script>"
end sub

sub deleteall()
	select case act
	case "period"
		conn.execute("createrhtml_soft_delete_period '"&timekind&"','"&cdate(replace(stime,"$"," "))&"','"&cdate(replace(etime,"$"," "))&"','"&cid&"'")
	case "user"
		conn.execute("createrhtml_soft_delete_user '"&uname&"','"&cid&"'")
	end select
end sub
%>