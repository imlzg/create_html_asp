...
sub softmoveAll()
	...
	case "period"
		...
		if recordcount=0 then
			Response.Write "没有SOFT需要转移！"
			Response.End
		else
			set rs=conn.execute("createrhtml_soft_get_sid_period '"&pageno&"','"&timekind&"','"&cdate(replace(stime,"$"," "))&"','"&cdate(replace(etime,"$"," "))&"','"&scid&"'")
		end if
	case "user"
		if isnumeric(recordcount)=0 or recordcount="" then
		...
		if recordcount=0 then
			...
			set rs=conn.execute("createrhtml_soft_get_sid_user '"&pageno&"','"&uname&"','"&scid&"'")
		end if
	end select

	if not (rs.eof and rs.bof) then
		response.write"<b><font color=#ff0000>正在转移SOFT的Html页,请等待......</font></b><span id=showImport></span><IE:Download ID=""oDownload"" STYLE=""behavior:url(#default#download)"" />"
		dim i,bn,en
		i=0
		do while not rs.eof
			sid=clng(rs("sid"))
	...
	if en>=clng(recordcount) then
		call moveall()
		Response.Write "<meta http-equiv=refresh content='0;url=soft_move_all.asp?act=ok&pageno="&en&"&time="&time&"'>"
	else
		Response.Write "<meta http-equiv=refresh content='0;url=soft_move_all.asp?act="&act&"&pageno="&pageno+1&"&recordcount="&recordcount&"&time="&time&"&scid="&scid&"&cid="&cid&"&s="&en
		select case act
		case "period"
...
sub success()
	Response.Write "<br>批量移动SOFT页<br><br>操作成功：共移动数据<font color=#ff0000>"&pageno&"</font>条，移动页面<font color=#ff0000>"&pageno&"</font>个，总费时<font color=#ff0000>"&formatnumber((timer()-time),2)&"</font>秒，完成时间"&now()
end sub

sub movehtml()
	Response.Write "<script>function onDownloadDone(downDate){showImport.innerHTML=downDate}oDownload.startDownload('soft_move.asp?sid="&sid&"&cid="&cid&"',onDownloadDone)</script>"
end sub

sub moveall()
	select case act
	case "period"
		conn.execute("createrhtml_soft_move_period '"&timekind&"','"&cdate(replace(stime,"$"," "))&"','"&cdate(replace(etime,"$"," "))&"','"&scid&"','"&cid&"'")
	case "user"
		conn.execute("createrhtml_soft_move_user '"&uname&"','"&scid&"','"&cid&"'")
	end select
end sub
%>