...
<%
	...
	Dvbbs.Execute("update [dv_user] set usergroupid=4 where username='"&replace(request("name"),"'","")&"'")
	body="<li>管理员删除成功。"
	end sub
...
''''''''''''''
'更新用户的的级别
        Dvbbs.Execute("update [dv_user] set usergroupid=1,userclass='"&groupsname&"',titlepic='"&titlepic&"' where username='"&trim(request("oldusername"))&"'")	'
